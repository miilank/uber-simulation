...
public class AdminDriverRidesService {
    ...
    public AdminDriverRidesService(@NonNull Context context) {
        this.adminApi = ApiClient.get().create(AdminApi.class);
    ...
    public void fetchRidesForDriver(@NonNull String driverEmail) {
        ...
        adminApi.getDriverRides("Bearer " + token, driverEmail).enqueue(new Callback<>() {
            @Override
    ...
    private void recompute(@NonNull List<DriverRideDto> list) {
        DriverRideDto next = pickCurrentRide(list);
        currentRide.postValue(next);
    }

    private DriverRideDto pickCurrentRide(@NonNull List<DriverRideDto> list) {
